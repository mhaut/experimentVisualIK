...
allerrorVisual = list()
#alltimeStamp = list()
allerrorVisualT = list()
allerrorVisualR = list()
for line in infile:
    if "target" not in line:
        ...
        errorVisualT.append(float(errorT[1]))
        errorVisualR.append(float(errorR[1]))
    else:
        if cont > 0:
            alltimeStamp.append(timeStamp)
            allerrorVisual.append(errorVisual)
            allerrorVisualT.append(errorVisualT)
            allerrorVisualR.append(errorVisualR)
        else:
            cont += 1
        timeStamp = list()
        errorVisual = list()
...
allOrderErrorVisualT = list()
allOrderErrorVisualR = list()
...
for i in range(len(allerrorVisual)):
    ...
    allOrderErrorVisualT += allerrorVisualT[i]
    allOrderErrorVisualR += allerrorVisualR[i]
#inds = allOrdertimeStamp.argsort()
#allOrderErrorVisual = allOrderErrorVisual[inds]

aux = allOrdertimeStamp
aux2 = aux
allOrdertimeStamp, allOrderErrorVisual = zip(*sorted(zip(allOrdertimeStamp,allOrderErrorVisual)))
aux, allOrderErrorVisualT = zip(*sorted(zip(aux,allOrderErrorVisualT)))
aux2, allOrderErrorVisualR = zip(*sorted(zip(aux2,allOrderErrorVisualR)))
...
fp, residuals, rank, sv, rcond = sp.polyfit(allOrdertimeStamp, allOrderErrorVisual, 4, full=True)
print("Model parameters of fp2: %s" % fp)
print("Error of the model of fp2:", residuals)
# contruimos la funcion
#f = sp.poly1d(fp)
#f1 = sp.poly1d(sp.polyfit(allOrdertimeStamp, allOrderErrorVisual, 1))
#f2 = sp.poly1d(sp.polyfit(allOrdertimeStamp, allOrderErrorVisual, 2))
#f3 = sp.poly1d(sp.polyfit(allOrdertimeStamp, allOrderErrorVisual, 3))
#f4 = sp.poly1d(sp.polyfit(allOrdertimeStamp, allOrderErrorVisual, 4))



f = sp.poly1d(sp.polyfit(allOrdertimeStamp, allOrderErrorVisual, 2))
ft = sp.poly1d(sp.polyfit(allOrdertimeStamp, allOrderErrorVisualT, 2))
fr = sp.poly1d(sp.polyfit(allOrdertimeStamp, allOrderErrorVisualR, 2))
fx = sp.linspace(allOrdertimeStamp[0]-15,allOrdertimeStamp[len(allOrdertimeStamp)-1]+15, num=300)


plt.subplot(2,2,1)
plt.plot(allOrdertimeStamp, allOrderErrorVisualR,'o')
plt.plot(fx, fr(fx), linewidth=2)
plt.subplot(2,2,2)
plt.plot(allOrdertimeStamp, allOrderErrorVisualT,'o')
plt.plot(fx, ft(fx), linewidth=2)
plt.subplot(2,1,2)
plt.plot(allOrdertimeStamp, allOrderErrorVisual,'o')
